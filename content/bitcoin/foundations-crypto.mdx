...
<Callout type="warning" title="Never invent crypto">
  Do not roll your own key derivation, RNG, or signature code. Use battle-tested
  libraries (`secp256k1`, `bitcoinjs-lib`, `rust-bitcoin`, BDK). A one-line RNG mistake
  can leak keys.
</Callout>
...
2. Public keys and addresses are safe to share for _receiving_.
3. Signatures authorize _specific_ messages (transaction sighashes), not blank checks.
